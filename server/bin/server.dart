...
/// billboard_life: 0=응답없음, 1=응답있음
int billboard_life = 0;
...
/// --------------------------
///  OverrideServer (CORS + 설정 업데이트 + override + isalive)
/// --------------------------
class OverrideServer {
  /// 10, 11, 20 등 override 값 저장
      ...
      await for (HttpRequest request in server) {
        try {
          ...
          if (request.uri.path == '/checkSetting') {
            ...
            await request.response.close();
          } else if (request.uri.path == '/setOverride' &&
              request.method == 'POST') {
            // POST {"value":"odd/even/original"}
            String body = await utf8.decoder.bind(request).join();
            try {
            ...
            await request.response.close();
          } else if (request.uri.path == '/isalive' &&
              request.method == 'GET') {
            // billboard_life 값 반환
            request.response.write('$billboard_life');
            await request.response.close();
          } else {
            // 그 외 라우트
...
class PacketSender {
  Future<void> sendPacket(
      Map<String, dynamic> settings, Uint8List packet) async {
    String ip = settings["IP"] ?? "192.168.0.217";
...
class TaskRunner {
  ...
  Future<void> run(
    ...
    try {
      ...
      // 백엔드 URL을 사용하여 POST 요청 전송
      HttpClient client = HttpClient();
      HttpClientRequest request = await client
          .postUrl(Uri.parse("${BackendConfig.backendUrl}/billboard"));
      ...
      for (int i = 0; i < lineList.length; i++) {
        ...
        if (item is Map && item.containsKey("text")) {
          ...
          String text = await getLineText(settingValue, settings, mainData,
              holidayManager, restrictionOverride, updateRestriction);
  ...
  Future<String> getLineText(
    ...
    if (parsed.lotParts.any((lot) => lot.trim().toLowerCase() == "5부제")) {
      return getFiveRestrictionText(
          restrictionOverride, updateRestriction, holidayManager);
    }
...
/// --------------------------
/// BackendConfig (assets/backend.json에서 백엔드 URL 불러오기)
/// --------------------------
class BackendConfig {
  static String backendUrl = "";

  static Future<void> loadBackendUrl() async {
    try {
      String content = await File('assets/backend.json').readAsString();
      Map<String, dynamic> jsonData = jsonDecode(content);
      backendUrl = jsonData["backend"] ?? "";
      print("Backend URL loaded: $backendUrl");
    } catch (e) {
...
class BillboardApp {
  ...
  Future<void> run() async {
    ...
    // 3-1) asset/packet.json에서 패킷 템플릿 로드 (한 번만 로드)
    await PacketUtils.loadPacketTemplate();

    // 3-2) asset/backend.json에서 백엔드 URL 로드 (한 번만 로드)
    await BackendConfig.loadBackendUrl();
